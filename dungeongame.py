...
class GameStateNode:
    def __init__(self, player_state, left=None, right=None):
...
class Dungeon:
                ...
                def save_game(self):
                    ...
                    new_state = copy.deepcopy(self.player)
                    new_node = GameStateNode(new_state)
                    if self.root is None:
                        self.root = new_node
                        self.current_node = self.root
                        print("Game state saved as root.")
                    else:
                        # Insert in the binary tree (simplified as always to the left)
                        if self.current_node.left is None:
                            self.current_node.left = new_node
                            print("Game state saved to the left branch.")
                        elif self.current_node.right is None:
                            self.current_node.right = new_node
                            print("Game state saved to the right branch.")
                        else:
                            print("No available branch to save; this should not happen.")
                        self.current_node = new_node
                    print("Current Node after save:", self.current_node)

                def load_game(self):
                    if self.root is None:
                        print("No saved game states available.")
                        return
                    print("Loading previous game state...")
                    self.current_node = self.root  # Start from the root
                    while True:
                        print("1: Load from the left branch")
                        print("2: Load from the right branch")
                        print("3: Use this game state")
                        choice = input("Choose an option: ")
                        if choice == "1" and self.current_node.left is not None:
                            self.current_node = self.current_node.left
                            print("Moved to left branch.")
                        elif choice == "2" and self.current_node.right is not None:
                            self.current_node = self.current_node.right
                            print("Moved to right branch.")
                        elif choice == "3":
                            self.player = copy.deepcopy(self.current_node.player_state)
                            print("Game state loaded.")
                            break
                        else:
                            print("Invalid choice or branch doesn't exist.")
                        print("Current Node after load attempt:", self.current_node)

                def play_game(self):
                    for floor in range(1, 51):
                        print(f"\n== Floor {floor} ==")
                        self.save_game()  # Save game state at the beginning of each floor
                        ...
                        if input("Do you want to load a previous game state? (y/n): ").lower() == 'y':
                            self.load_game()
